...
_rotr.apis.getUserMsg = function () {
    var ctx = this;
    var co = $co(function* () {
        var sqlstr = "SELECT msgid,request,requserid,flag,feedback,feeduserid,nick FROM sysmsg s LEFT JOIN user_info u ON s.requserid=u.userid";
        var rows = yield _ctnu([_Mysql.conn, 'query'], sqlstr);
...
_rotr.apis.getNotice = function () {
    ...
    var co = $co(function* () {
        ...
        if (role == '老师' || role == '学生') {
            var sqlstr1 = 'select * from sysmsg  INNER  JOIN user_info  where requserid=userid and requserid=' + userid + ' and flag=1;';
            var rows1 = yield _ctnu([_Mysql.conn, "query"], sqlstr1);
        } else {
            rows1 = [];
        }
...
_rotr.apis.delete = function () {
    ...
    var co = $co(function* () {
        if ((ctx.query.wid || ctx.request.body.wid) != null) {
            ...
            if ((chatid = ctx.query.chatid || ctx.request.body.chatid) != null) {
                var chatid = ctx.query.chatid || ctx.request.body.chatid;
