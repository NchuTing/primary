		...
		$.get('/homework/api/kuWorkDetail', dat, function (res) {
			...
			$.get('/homework/api/SWorkDetail', dat, function (res) {
				...
				$scope.update = function () {
					console.log(':', score);
					console.log("dsds", res.data[0].annex, res.data[0].answer)
				...
				if (dat1.content == "") {
					...
					$.post('/homework/api/ChatContent', dat1, function (res) {
						...
						if (res.data.affectedRows == 1) {
							...
							boxshow();
							$('#chat').val("");
							$.post('/homework/api/openChat', dat, function (res) {
								res.data[0].bg = arry[0];
								console.log('kkkkkkk', res.data[0])
								$scope.$apply(function () {
									$scope.obj.splice(0, 0, res.data[0]);//在数组最前面添加元素
									//Array.prototype.unshift.apply($scope.obj, temp);//在数组的最前面添加数组
								})
								$('#none').hide();
							})
						} else {}
